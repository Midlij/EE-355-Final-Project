...
bool Date::check_date(){
	...
	switch (month){
		...
		case Feb:
            if (day > 29)
                return false;
            /*else if (day == 28 && year%4 == 0){     // 29 not 28
                if (year%100 == 0 && year%400 != 0)  //  ??  ->400 !=0  not ==0
                    return false;
                else
                    return true;
            }*/
...
bool Date::check_pal_date()
{
    ...
    // Suppose check the format of MonthDayYear
    temp = to_string(month) + to_string(day) + to_string(year); //to_string(this->year);
    start = 0;
    end = temp.length()-1;
    while (end > start) {
